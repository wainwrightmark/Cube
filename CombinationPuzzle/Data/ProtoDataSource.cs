...
{
    public sealed class ProtoDataSource : DataSource
    {

        /// <inheritdoc />
        public ProtoDataSource()
        {
            var sw = Stopwatch.StartNew();
            Console.WriteLine("Beginning Loading of Cube Data");

            ReadOnlyMemory<byte> byteData = CubeDataResource.ProtoData;

            Console.WriteLine($"Beginning Deserialization of Cube Data ({sw.Elapsed})");

            var cubeData = Serializer.Deserialize<ProtoBufCubeData>(byteData);
